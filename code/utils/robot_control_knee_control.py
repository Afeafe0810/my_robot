...
class Outterloop:
    @staticmethod
    def left_leg_jacobian(frame:ULC_frame):
        pelvis = copy.deepcopy(frame.P_PV_pf)
        l_hip_roll = copy.deepcopy(frame.P_Lhr_pf)
        l_hip_yaw = copy.deepcopy(frame.P_Lhy_pf)
        l_hip_pitch = copy.deepcopy(frame.P_Lhp_pf)
        l_knee_pitch = copy.deepcopy(frame.P_Lkp_pf)
        l_ankle_pitch = copy.deepcopy(frame.P_Lap_pf)
        l_ankle_roll = copy.deepcopy(frame.P_Lar_pf)
        l_foot = copy.deepcopy(frame.P_L_pf)

        JL1 = np.cross(frame.AL1,(l_foot-l_hip_roll),axis=0)
        JL2 = np.cross(frame.AL2,(l_foot-l_hip_yaw),axis=0)
        JL3 = np.cross(frame.AL3,(l_foot-l_hip_pitch),axis=0)
        JL4 = np.cross(frame.AL4,(l_foot-l_knee_pitch),axis=0)
        JL5 = np.cross(frame.AL5,(l_foot-l_ankle_pitch),axis=0)
        JL6 = np.cross(frame.AL6,(l_foot-l_ankle_roll),axis=0)

        JLL_upper = np.hstack((JL1, JL2,JL3,JL4,JL5,JL6))
        JLL_lower = np.hstack((frame.AL1,frame.AL2,frame.AL3,frame.AL4,frame.AL5,frame.AL6))    
        frame.JLL = np.vstack((JLL_upper,JLL_lower))  
        # print(frame.JLL)

        #排除支撐腳腳踝對末端速度的影響
        frame.JL_sp44 = np.reshape(frame.JLL[2:,0:4],(4,4))  
        frame.JL_sp42 = np.reshape(frame.JLL[2:,4:],(4,2))
        #排除擺動.腳踝對末端速度的影響
        JL_sw34 = np.reshape(frame.JLL[0:3,0:4],(3,4)) 
        JL_sw14 = np.reshape(frame.JLL[5,0:4],(1,4)) 
        frame.JL_sw44 = np.vstack((JL_sw34,JL_sw14))

        JL_sw32 = np.reshape(frame.JLL[0:3,4:],(3,2)) 
        JL_sw12 = np.reshape(frame.JLL[5,4:],(1,2)) 
        frame.JL_sw42 = np.vstack((JL_sw32,JL_sw12))

        return frame.JLL

    @staticmethod
    def right_leg_jacobian(frame:ULC_frame):
        pelvis = copy.deepcopy(frame.P_PV_pf)
        r_hip_roll = copy.deepcopy(frame.P_Rhr_pf)
        r_hip_yaw = copy.deepcopy(frame.P_Rhy_pf)
        r_hip_pitch = copy.deepcopy(frame.P_Rhp_pf)
        r_knee_pitch = copy.deepcopy(frame.P_Rkp_pf)
        r_ankle_pitch = copy.deepcopy(frame.P_Rap_pf)
        r_ankle_roll = copy.deepcopy(frame.P_Rar_pf)
        r_foot = copy.deepcopy(frame.P_R_pf)

        JR1 = np.cross(frame.AR1,(r_foot-r_hip_roll),axis=0)
        JR2 = np.cross(frame.AR2,(r_foot-r_hip_yaw),axis=0)
        JR3 = np.cross(frame.AR3,(r_foot-r_hip_pitch),axis=0)
        JR4 = np.cross(frame.AR4,(r_foot-r_knee_pitch),axis=0)
        JR5 = np.cross(frame.AR5,(r_foot-r_ankle_pitch),axis=0)
        JR6 = np.cross(frame.AR6,(r_foot-r_ankle_roll),axis=0)

        JRR_upper = np.hstack((JR1,JR2,JR3,JR4,JR5,JR6))
        JRR_lower = np.hstack((frame.AR1,frame.AR2,frame.AR3,frame.AR4,frame.AR5,frame.AR6))    
        frame.JRR = np.vstack((JRR_upper,JRR_lower))  
        # print(frame.JRR)

        #排除支撐腳腳踝對末端速度的影響
        frame.JR_sp44 = np.reshape(frame.JRR[2:,0:4],(4,4))  
        frame.JR_sp42 = np.reshape(frame.JRR[2:,4:],(4,2))
        #排除擺動.腳踝對末端速度的影響
        JR_sw34 = np.reshape(frame.JRR[0:3,0:4],(3,4)) 
        JR_sw14 = np.reshape(frame.JRR[5,0:4],(1,4)) 
        frame.JR_sw44 = np.vstack((JR_sw34,JR_sw14))

        JR_sw32 = np.reshape(frame.JRR[0:3,4:],(3,2)) 
        JR_sw12 = np.reshape(frame.JRR[5,4:],(1,2)) 
        frame.JR_sw42 = np.vstack((JR_sw32,JR_sw12))

        return frame.JRR

    @staticmethod
    def calculate_err(ulc, frame:ULC_frame, state):
        PX_ref = copy.deepcopy(ulc.PX_ref) #wf
        LX_ref = copy.deepcopy(ulc.LX_ref) #wf
        RX_ref = copy.deepcopy(ulc.RX_ref) #wf
        PX = copy.deepcopy(frame.PX) #pf
        LX = copy.deepcopy(frame.LX) #pf
        RX = copy.deepcopy(frame.RX) #pf
        state = copy.deepcopy(state)
        
        #foot_trajectory(by myframe)
        L_ref = LX_ref - PX_ref 
        R_ref = RX_ref - PX_ref
        ...
        # #--PI
        # Le_dot = frame.Le_dot_past + 20*Le - 19.99*frame.Le_past 
        # frame.Le_dot_past = Le_dot
        # frame.Le_past = Le

        # Re_dot = frame.Re_dot_past + 20*Re - 19.99*frame.Re_past 
        # frame.Re_dot_past = Re_dot
        # frame.Re_past = Re
        ...
        WL_x = frame.L_Body_transfer[0,0]*Lroll_error_dot + frame.L_Body_transfer[0,1]*Lpitch_error_dot
        WL_y = frame.L_Body_transfer[1,0]*Lroll_error_dot + frame.L_Body_transfer[1,1]*Lpitch_error_dot
        WL_z = frame.L_Body_transfer[2,0]*Lroll_error_dot + frame.L_Body_transfer[2,2]*Lyaw_error_dot
        ...
        WR_x = frame.R_Body_transfer[0,0]*Rroll_error_dot + frame.R_Body_transfer[0,1]*Rpitch_error_dot
        WR_y = frame.R_Body_transfer[1,0]*Rroll_error_dot + frame.R_Body_transfer[1,1]*Rpitch_error_dot
        WR_z = frame.R_Body_transfer[2,0]*Rroll_error_dot + frame.R_Body_transfer[2,2]*Ryaw_error_dot
...
class Innerloop:
    @staticmethod
    def gravity_compemsate(ulc,joint_position,stance_type,px_in_lf,px_in_rf,l_contact,r_contact,state):
        ...
        c_L_DS = np.zeros((6,1))
        L_DS_gravity = np.reshape(-pin.rnea(ulc.stance_l_model, ulc.stance_l_data, jp_L_DS,jv_L_DS,(c_L_DS)),(6,1))  
        L_DS_gravity = np.flip(L_DS_gravity,axis=0)

        jp_R_DS = np.flip(-jp_r,axis=0)
        jv_R_DS = np.zeros((6,1))
        c_R_DS = np.zeros((6,1))
        R_DS_gravity = np.reshape(-pin.rnea(ulc.stance_r_model, ulc.stance_r_data, jp_R_DS,jv_R_DS,(c_R_DS)),(6,1))  
        ...
        c_RSS = np.zeros((12,1))
        Leg_RSS_gravity = np.reshape(pin.rnea(ulc.bipedal_r_model, ulc.bipedal_r_data, jp_RSS,jv_RSS,(c_RSS)),(12,1))  
        ...
        c_LSS = np.zeros((12,1))
        Leg_LSS_gravity = np.reshape(pin.rnea(ulc.bipedal_l_model, ulc.bipedal_l_data, jp_LSS,jv_LSS,(c_LSS)),(12,1))  
        ...
        ulc.publisher["gravity_l"].publish(Float64MultiArray(data=l_leg_gravity))
        ulc.publisher["gravity_r"].publish(Float64MultiArray(data=r_leg_gravity))
        
        return l_leg_gravity,r_leg_gravity,kl,kr
    
    @staticmethod
    def velocity_cmd(frame:ULC_frame,Le_2,Re_2,jv_f,stance_type,state):
        ...
        if state == 1:
            if stance == 0:   
                ...
                #計算右膝關節以上速度
                R2_41_cal = R2_41 - frame.JR_sp42@VR56
                #彙整右腳速度
                rw_41_d = np.dot(np.linalg.pinv(frame.JR_sp44),R2_41_cal)
                ...
                #計算左膝關節以上速度
                L2_41_cal = L2_41 - frame.JL_sw42@VL56
                #彙整左腳速度
                lw_41_d = np.dot(np.linalg.pinv(frame.JL_sw44),L2_41_cal)
                lw_21_d = np.zeros((2,1))
                Lw_d = np.vstack((lw_41_d,lw_21_d))

                # Lw_d = np.dot(np.linalg.pinv(frame.JLL),L2) 
                ...
                VL56 =  np.reshape(v[4:6,0],(2,1)) #左腳腳踝速度
                #計算左膝關節以上速度
                L2_41_cal = L2_41 - frame.JL_sp42@VL56
                #彙整左腳速度
                lw_41_d = np.dot(np.linalg.pinv(frame.JL_sp44),L2_41_cal)
                lw_21_d = np.zeros((2,1))
                Lw_d = np.vstack((lw_41_d,lw_21_d))
                ...
                VR56 =  np.reshape(v[10:,0],(2,1)) #右腳腳踝速度
                #計算右膝關節以上速度
                R2_41_cal = R2_41 - frame.JR_sw42@VR56
                #彙整右腳速度
                rw_41_d = np.dot(np.linalg.pinv(frame.JR_sw44),R2_41_cal)
                rw_21_d = np.zeros((2,1))
                Rw_d = np.vstack((rw_41_d,rw_21_d))
                # Rw_d = np.dot(np.linalg.pinv(frame.JRR),R2) 
        else:
            Lw_d = np.dot(np.linalg.pinv(frame.JLL),L2) 
            Rw_d = np.dot(np.linalg.pinv(frame.JRR),R2) 
    ...
    #     node.publisher["effort"].publish(Float64MultiArray(data=torque))
    ...
    def swing_leg(node,joint_velocity,l_leg_vcmd,r_leg_vcmd,l_leg_gravity_compensate,r_leg_gravity_compensate,kl,kr):
        ...
        node.publisher["vcmd"].publish(Float64MultiArray(data=vcmd_data))
        jv_collect = np.array([[jv[0,0]],[jv[1,0]],[jv[2,0]],[jv[3,0]],[jv[4,0]],[jv[5,0]]])
        node.publisher["velocity"].publish(Float64MultiArray(data=jv_collect))#檢查收到的速度(超髒)
    ...
    def walking_by_ALIP(node,joint_velocity,l_leg_vcmd,r_leg_vcmd,l_leg_gravity_compensate,r_leg_gravity_compensate,kl,kr):
        ...
        L_matrix = R.from_matrix(node.pt.O_wfL)
        L_euler = L_matrix.as_euler('zyx', degrees=False)
        L_yaw = L_euler[0]
        L_pitch = L_euler[1]
        L_roll = L_euler[2]

        R_matrix = R.from_matrix(node.pt.O_wfR)
    ...
    def alip_L(node,stance_type,px_in_lf,torque_ALIP,com_in_lf,state):
        ...
        com_in_wf = copy.deepcopy(node.pt.P_COM_wf)
        lx_in_wf = copy.deepcopy(node.pt.P_L_wf)
        ...
        #計算質心速度(v從世界座標下求出)
        node.pt.CX_dot_L = (com_in_wf[0,0] - node.pt.CX_past_L)/node.timer_period
        node.pt.CX_past_L = com_in_wf[0,0]
        node.pt.CY_dot_L = (com_in_wf[1,0] - node.pt.CY_past_L)/node.timer_period
        node.pt.CY_past_L = com_in_wf[1,0]

        #velocity filter
        node.pt.Vx_L = 0.7408*node.pt.Vx_past_L + 0.2592*node.pt.CX_dot_past_L  #濾過後的速度(5Hz)
        node.pt.Vx_past_L = node.pt.Vx_L
        node.pt.CX_dot_past_L =  node.pt.CX_dot_L

        node.pt.Vy_L = 0.7408*node.pt.Vy_past_L + 0.2592*node.pt.CY_dot_past_L  #濾過後的速度(5Hz)
        node.pt.Vy_past_L = node.pt.Vy_L
        node.pt.CY_dot_past_L =  node.pt.CY_dot_L

        #量測值
        Xc_mea = PX_l[0,0]
        Ly_mea = 9*node.pt.Vx_L*0.45
        Yc_mea = PX_l[1,0]
        Lx_mea = -9*node.pt.Vy_L*0.45 #(記得加負號)
        node.pt.mea_x_L = np.array([[Xc_mea],[Ly_mea]])
        node.pt.mea_y_L = np.array([[Yc_mea],[Lx_mea]])
        ...
        if node.stance_past == 0 and node.stance == 1:
            node.pt.mea_x_L[1,0] = copy.deepcopy(node.pt.mea_x_past_R[1,0])
        ...
        node.ap_L = -Kx@(node.pt.mea_x_L - ref_x_L)
        ...
        #----update
        # node.pt.mea_x_past_L = node.pt.mea_x_L
        # node.pt.ob_x_past_L = node.pt.ob_x_L
        ...
        #為了確保支撐腳切換過程 角動量估測連續性
        if node.stance_past == 0 and node.stance == 1:
            node.pt.mea_y_L[1,0] = copy.deepcopy(node.mea_y_past_R[1,0])
        ...
        node.ar_L = -Ky@(node.pt.mea_y_L - ref_y_L)
        ...
        # torque[5,0] = 0
        #----update
        node.pt.mea_y_past_L = node.pt.mea_y_L
        # node.ob_y_past_L = node.ob_y_L
        node.ar_past_L = node.ar_L

        # node.effort_publisher.publish(Float64MultiArray(data=torque))
        tl_data= np.array([[torque[4,0]],[torque[5,0]]])
        node.publisher["torque_l"].publish(Float64MultiArray(data=tl_data))
        ...
        if stance == 1:
            ...
            node.publisher["alip_x"].publish(Float64MultiArray(data=alip_x_data))
            node.publisher["alip_y"].publish(Float64MultiArray(data=alip_y_data))
    ...
    #     com_in_wf = copy.deepcopy(node.pt.P_COM_wf)
    #     rx_in_wf = copy.deepcopy(node.pt.P_R_wf)
    #     PX_r = com_in_wf - rx_in_wf
       
    #     #計算質心速度
    #     node.pt.CX_dot_R = (com_in_wf[0,0] - node.pt.CX_past_R)/node.timer_period
    #     node.pt.CX_past_R = com_in_wf[0,0]
    #     node.pt.CY_dot_R = (com_in_wf[1,0] - node.pt.CY_past_R)/node.timer_period
    #     node.pt.CY_past_R = com_in_wf[1,0]

    #     #velocity filter
    #     node.pt.Vx_R = 0.7408*node.pt.Vx_past_R + 0.2592*node.pt.CX_dot_past_R  #濾過後的速度(5Hz)
    #     node.pt.Vx_past_R = node.pt.Vx_R
    #     node.pt.CX_dot_past_R =  node.pt.CX_dot_R

    #     node.pt.Vy_R = 0.7408*node.pt.Vy_past_R + 0.2592*node.pt.CY_dot_past_R  #濾過後的速度(5Hz)
    #     node.pt.Vy_past_R = node.pt.Vy_R
    #     node.pt.CY_dot_past_R =  node.pt.CY_dot_R

    #     #量測值
    #     Xc_mea = PX_r[0,0]
    #     Ly_mea = 9*node.pt.Vx_R*0.45
    #     Yc_mea = PX_r[1,0]
    #     Lx_mea = -9*node.pt.Vy_R*0.45 #(記得加負號)
    #     node.pt.mea_x_R = np.array([[Xc_mea],[Ly_mea]])
    #     node.pt.mea_y_R = np.array([[Yc_mea],[Lx_mea]])
    ...
    #         node.pt.mea_x_R[1,0] = copy.deepcopy(node.pt.mea_x_past_L[1,0])
