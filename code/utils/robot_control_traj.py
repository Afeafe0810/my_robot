...
def __comMoveTolf(DS_time, DDT):
    ref_p_lf_in_wf  = np.vstack(( 0,  0.1,  0, 0, 0, 0 ))
    ref_p_rf_in_wf  = np.vstack(( 0, -0.1,  0, 0, 0, 0 ))
    
    ref_p_pel_in_wf = np.vstack(( 0, 0.06*DS_time/( 0.5*DDT ), 0.55, 0, 0, 0 )) if 0 < DS_time <= 0.5*DDT else \
