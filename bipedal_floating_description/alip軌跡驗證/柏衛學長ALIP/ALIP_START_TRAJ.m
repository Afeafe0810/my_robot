...
% Lxt_yaw = 0.125*(m*H*l*S)*W/(1+C);
Lxt_yaw = 0.075*(m*H*l*S)*W/(1+C);
% Lxt_yaw=0;
xt_collect=[xt];
yt_collect=[yt];
...
plot(polyp(i), 'FaceColor', 'w', 'EdgeColor', color(2), 'LineStyle', '-');
...
for i=1:length(lx_collect)
    ...
    plot3(lf_x_collect(i),lf_y_collect(i),lf_z_collect(i),'.-','Color','b',"LineWidth",0.5,'MarkerSize',10)
    plot3(rf_x_collect(i),rf_y_collect(i),rf_z_collect(i),'.-','Color','r',"LineWidth",0.5,'MarkerSize',10)
    CL = line([xc_collect(i); lf_x_collect(i)], [yc_collect(i); lf_y_collect(i)], [H;lf_z_collect(i)], 'Color', 'b');
    CR = line([xc_collect(i); rf_x_collect(i)], [yc_collect(i); rf_y_collect(i)], [H;rf_z_collect(i)], 'Color', 'r');
